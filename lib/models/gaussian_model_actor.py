...
class GaussianModelActor(GaussianModel):
    ...
    def get_features_fourier(self, frame=0):
        ...
        参数：
        - frame (int 或 float): 当前帧索引，表示时间。

        返回：
        - features (torch.Tensor): 形状 [N, (sh + 1), 3]，融合了傅里叶时间特征和空间特征。
        """
        # 归一化时间
        normalized_frame = (frame - self.start_frame) / (self.end_frame - self.start_frame)
        time = self.fourier_scale * normalized_frame  # 时间缩放

        # 计算傅里叶基
        idft_base = IDFT(time, self.fourier_dim)[0].cuda()  # 获取 IDFT 矩阵并放入 GPU
        ...
        # 拼接最终特征
        features = torch.cat([features_dc, features_rest], dim=1)  # [N, (sh + 1), 3]
