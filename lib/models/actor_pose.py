...
class ActorPose(nn.Module):      
    def __init__(self, tracklets, tracklet_timestamps, camera_timestamps, obj_info):
        ...
        self.opt_track = cfg.model.nsg.opt_track  # 从配置文件中获取是否进行轨迹优化的标志
        if self.opt_track:  # 如果启用了轨迹优化

            # 创建一个可训练参数 opt_trans，用于存储平移优化量
            # 初始化为与 input_trans 形状相同的全零张量
            self.opt_trans = nn.Parameter(torch.zeros_like(self.input_trans)).requires_grad_(True)
            # opt_trans 形状：[num_frames, max_obj, [dx, dy, dz]]
            # 其中 num_frames 代表帧数，max_obj 代表最大物体数，dx/dy/dz 代表每个物体的平移偏移量

            # 创建一个可训练参数 opt_rots，用于存储旋转优化量
            # 初始化为与 input_rots[..., :1] 形状相同的全零张量，仅优化单个旋转维度（例如绕 Z 轴的角度变化）
            self.opt_rots = nn.Parameter(torch.zeros_like(self.input_rots[..., :1])).requires_grad_(True)
            # opt_rots 形状：[num_frames, max_obj, [dtheta]]
            # 其中 num_frames 代表帧数，max_obj 代表最大物体数，dtheta 代表旋转角度的变化量
